...
A Zsh plugin for simplified AWS profile management, with `aws-vault` and `tmux` integration.
...
1.  **If `$AWS_PROFILE` is set**: The plugin will use `aws-vault` to export temporary credentials for the specified profile into your shell session.
...
2.  **If `$AWS_PROFILE` is NOT set, but in a `tmux` session**: The plugin checks for a `tmux` window option named `@AWS_PROFILE`.

    -   If `@AWS_PROFILE` is set for the window, its value is used as the `AWS_PROFILE`.
    -   `aws-vault` then exports credentials for that profile.
...
To use the `tmux` integration, set the `@AWS_PROFILE` window option. You can do this in your `.tmux.conf` or manually per window.
